...
def laplace(latent_vars=None, data=None,
  ...
  return map(latent_vars, data,
             auto_transform, scale, var_list, summary_key)


# TODO
# + if passed in list (also in MAP), how to get latent_vars back out?
#   + same as how to get posterior_trace in 2.0
# + API for finalize
# + how to pass in finalize fn in run()?
# + how to pass in finalize args/kwargs in run()?
# + how to access latent_vars, latent_vars_normal distinction?
def finalize(latent_vars, latent_vars_normal, loss):
  ...
  latent_vars = latent_vars_normal.copy()
  return finalize_ops
