...
class Trace(object):
  ...
  def __getitem__(self, key):
    # for convenience, get node.value from stack, not node itself
    return self._trace_stack[key].value

  def __setitem__(self, key, value):
    self._trace_stack[key] = value

  def get(self, key, value=None):
    return self._trace_stack.get(key, value).value
  ...
  def values(self):
    return [v.value for v in self._trace_stack.values()]
...
def primitive_cls(__init__):
  ...
  def wrapped_fn(self, *args, **kwargs):
    global _INTERCEPT, _TRACE_CONTINUATION, _TRACE_STACK
    if '_INTERCEPT' in globals():
      self = _INTERCEPT(__init__, self, *args, **kwargs)
    else:
